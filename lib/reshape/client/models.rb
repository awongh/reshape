  ...
  class Client
    module Models
      ...
      def add_model(file, name, rights, terms, options = {}, raw = false)
        post("/models/#{api_version}",
             options.merge(file: file, fileName: name, hasRightsToModel: rights, acceptTermsAndConditions: terms), raw)
      end
      ...
      def add_model_file(id, file, name, rights, terms, options = {}, raw = false)
        post("/models/#{id}/files/#{api_version}",
             options.merge(file: file, fileName: name, hasRightsToModel: rights, acceptTermsAndConditions: terms), raw)
      end

      def model_file_by_version(id, file_version, _options = {}, _raw = false)
        get("/models/#{id}/files/#{file_version}/v1")
      end

      def add_model_photo(id, _file, _options = {}, _raw = false)
